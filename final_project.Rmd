...
my_lm = function(response, covariates, alpha, method) {
  ...
  if(nrow(response) != nrow(covariates)) stop('The number of rows of the response and predictor variables must be equal.')
  
  # Check to see if alpha is between 0 and 1
  if(alpha >= 1 | alpha <= 0) stop('Alpha must be between 0 and 1 (not inclusive) to create appropriate confidence intervals.')
  ...
  if(method != "asymptotic" & method != "bootstrap") stop('Unrecognized confidence interval method. Try "asymptotic" or "bootstrap".')
