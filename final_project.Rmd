...
my_lm = function(response, covariates, alpha=0.05, method="asymptotic", plots=TRUE) {
  ...
  # Check to see if plots is TRUE or FALSE only
  if(plots != TRUE & tolower(plots) != "true" & tolower(plots) != "t" & plots != FALSE & tolower(plots) != "false" & tolower(plots) != "f")
    stop('Please specify if you would like plots by entering TRUE or FALSE.')
  ...
  if(plots == TRUE | tolower(plots) == "t" | tolower(plots) == "true"){
    resid_plot <- plot(y.hat, resid)
    norm_plot <- qqnorm(resid)
    norm_plot <- qqline(resid, col = "red", lwd = 2)
    hist_plot <- hist(resid)
    #return(list(resid_plot, norm_plot, hist_plot))
  }
