...
<br><br><br>
We can also produce descriptive plot of our residuals using `plot_func()`. The paramater needed in `plot_func()` is the name of the object created when using the `my_lm()` function. 
<br><br>
In the example above we called our object `fit_my_lm` - so this is what will be used as the paramater in `plot_func`. 
<br><br>
...
We can also use the `my_lm()` function to perform multiple regressions. Compared to simple linear regressions, performed in the example above, multiple linear regressions are use to predict the value of response variable when two or more covariates are present. 
<br><br><br>
```{r example iris}
library(macklinear)
...
```
<br><br><br>
To get our descriptive plots, we can use `plot_func()` with the paramater again the object we called our `my_lm()` function.
``` {r example iris plot}
